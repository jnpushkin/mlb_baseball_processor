...
## Companion connection and attainable collections

The editor connection now survives opening another tab in the same browser while the manager runs. Authentication remains required; the manager uses an HttpOnly/SameSite=Strict session cookie and rejects cookie-authenticated requests from foreign origins. Direct HTML-file openings show a manager link without attempting file-based API requests. Verified that a fresh Chrome tab loaded all 296 games without reentering a token.

Collections now open to **Available goals**: only sets with unseen players verified on active MLB rosters. Goal denominators include seen winners plus active unseen winners; other unseen entries remain in **All history**. Completed collections remain accessible separately. This removes 1800s/1900s classes and retired-only sets such as Delivery Man of the Year from actionable goals. Historical award dates alone never exclude an active player or an actual sighting. Players absent from active rosters are not labeled retired; injured, unsigned and minor-league players can become targets when their roster status changes.

The September 21 roster snapshot checked 841 active players across all 30 MLB teams and matched 839 to local player IDs. The award archive offers **34 available collections out of 191 historical sets**. The same eligibility applies to overview recommendations, award/All-Star drill-ins, and candidates from previously pinned collections. Unknown or stale roster snapshots produce no current goal suggestions. Normal website processing refreshes rosters daily; a failed or partial refresh preserves the prior snapshot. The browser also checks snapshot age so an old static build cannot indefinitely advertise stale targets.

Validation: **261 Python tests passed, 2 skipped, 17 subtests passed; 30 browser tests passed**. Regression cases include old and retired-only sets, repeated award entries for a single active player, historical-count preservation, scoped checklist members, existing retired pins, stale snapshots, partial refresh failures, cross-origin authentication rejection, fresh-tab sessions and file-opening guidance. Ruff passed; the 366-file production release passed dependency/checksum validation. Visual review used the full local archive in dark mode.
