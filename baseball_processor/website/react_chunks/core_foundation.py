...
const getGameSourceProfile = (game, rowCounts = {}) => {
    const source = String(game?.source || 'unknown').toLowerCase();
    const hasMlbPk = !!game?.mlbGamePk;
    const hasLinescore = !!game?.linescore;
    const hasPbp = (game?.playByPlay || []).length > 0;
    const hasPitchData = game?.pitchData && Object.keys(game.pitchData).length > 0;
    const hasHitData = game?.hitData && Object.keys(game.hitData).length > 0;
    const hasLineups = !!game?.lineups;
    const hasAbs = !!game?.absChallenges;
    const batterRows = rowCounts.batterRows ?? null;
    const pitcherRows = rowCounts.pitcherRows ?? null;

    const strengths = [];
    const gaps = [];
    if (source === 'mlb') strengths.push('API primary');
    else if (source === 'bref') strengths.push('BREF primary');
    else gaps.push('Unknown source');
    if (hasMlbPk) strengths.push('MLB gamePk');
    else gaps.push('No MLB gamePk');
    if (hasLinescore) strengths.push('line score');
    else gaps.push('No line score');
    if (hasPbp) strengths.push('play-by-play');
    else gaps.push('No play-by-play');
    if (hasPitchData) strengths.push('pitch data');
    if (hasHitData) strengths.push('batted-ball data');
    if (hasLineups) strengths.push('lineups');
    if (hasAbs) strengths.push('ABS');
    if (batterRows !== null && batterRows === 0) gaps.push('No batting rows');
    if (pitcherRows !== null && pitcherRows === 0) gaps.push('No pitching rows');

    const score = [
        source === 'mlb' || source === 'bref',
        hasMlbPk,
        hasLinescore,
        hasPbp,
        batterRows === null || batterRows > 0,
        pitcherRows === null || pitcherRows > 0,
        hasPitchData,
        hasHitData,
    ].filter(Boolean).length;
    const level = score >= 7 ? 'high' : score >= 5 ? 'medium' : 'review';
    const label = source === 'mlb' ? 'API' : source === 'bref' ? 'BREF' : 'Unknown';
    return { source, label, level, strengths, gaps, score, hasMlbPk, hasLinescore, hasPbp, hasPitchData, hasHitData, hasLineups, hasAbs };
};

const SourceBadge = ({ game, profile, compact = false }) => {
    const p = profile || getGameSourceProfile(game);
    const tone =
        p.level === 'high' ? 'bg-emerald-100 text-emerald-800 border-emerald-200' :
        p.level === 'medium' ? 'bg-blue-100 text-blue-800 border-blue-200' :
        'bg-amber-100 text-amber-800 border-amber-200';
    const title = [
        `Source: ${p.label}`,
        p.strengths.length ? `Has: ${p.strengths.join(', ')}` : '',
        p.gaps.length ? `Review: ${p.gaps.join(', ')}` : '',
    ].filter(Boolean).join('\n');
    return (
        <span title={title} className={`inline-flex items-center gap-1 rounded border px-2 py-0.5 text-[10px] font-bold uppercase tracking-wide ${tone}`}>
            <span>{p.label}</span>
            {!compact && <span className="opacity-70">{p.level}</span>}
        </span>
    );
};
