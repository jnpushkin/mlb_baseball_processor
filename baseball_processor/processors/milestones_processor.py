    ...
    def _add_milestone(self, milestone_tabs, tab, basic_info, item, detail, max_innings):
        ...
        try:
            ...
            if tab in pitching_tabs:
                ...
                # Note: engine uses 'runs' and 'walks', not 'runs_allowed' and 'walks_allowed'
                pitching_stats = {
                    "IP": item.get("innings_pitched", "0.0"),
                    "H": item.get("hits", 0),
                    "R": item.get("runs", item.get("runs_allowed", 0)),
                    "ER": item.get("earned_runs", 0),
                    "BB": item.get("walks", item.get("walks_allowed", 0)),
...
def integrate_practical_enhancements(milestone_dfs, games):
    ...
    enhancements = {
        # Batting milestones - use appropriate enhancer patterns
        "3+ HR Games": enhancer.enhance_multi_hr_games,
        ...
        "Perfect Games": enhancer.enhance_quality_starts,
        "Quality Starts": enhancer.enhance_quality_starts,
        "15+ K Games": enhancer.enhance_ten_k_games,
        "12+ K Games": enhancer.enhance_ten_k_games,
