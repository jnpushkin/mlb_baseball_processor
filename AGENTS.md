...
If another application occupies the fixture port (Okta Verify can use 8769), run `MLB_BROWSER_TEST_PORT=8771 npm run test:browser` on a free port. Never reuse an unrelated listener as the test site or stop that application; Playwright must start its own fixture server.
...
- Threshold summaries must include every qualifying tier: `4+ Hit Games` must include five-hit performances, `5+ RBI Games` must include six-plus RBI performances, and `10+ K Games` must include twelve-plus strikeout performances. Do not use the length of a single tiered milestone sheet as an inclusive threshold count. Derive summary counts and linked games from the same eligible performance set, with an explicit spring-training policy.
- `website/record_book.py` builds a dedicated `recordBook` library at release time from serialized game facts. Stable IDs, units, direction, and per-game candidates support top-five rankings, ties, history, and recomputed season/team/venue/companion scopes. Preserve all ties at the fifth-place cutoff and the next distinct mark in each game; a single lifetime maximum cannot answer scoped queries. Legacy WPA has only a saved holder, so it explicitly has no filtered rankings/history. Records default to regular season plus postseason; spring training is a separate scope. Summary counts live at `#milestones/counts`, and archive totals at `#dashboard/totals`.
...
- MLB historical boxscores and dated rosters can retain erroneous #42 assignments outside tribute games (Iannetta 2018-05-26, Givens 2019-07-02/03, Pederson 2023-04-08). Use `utils.jerseys.game_jersey` for both the collection and game lineups: infer league-wide April 15 tributes only from 2009 onward, preserve grandfathered historical wearers, and withhold unsupported #42 after Rivera's 2013 retirement. Delayed/team-specific observances need a cited `uniform_tributes[side]` record. Never replace historical numbers with a person's current `primaryNumber`. Raw caches retain source values; `npm run build:website -- --refresh-jerseys` refreshes the published uniform data without reprocessing other statistics.
...
- The Record Book separates single-game records, occurrence counts, and archive summaries. Its latest-game spotlight must use actual record entries with linked games, not cumulative totals or counts. Build dates, venue searches, and tie counts from exact game IDs; retain all tied games. Format time/count units for display without coercing WPA, clock times, or temperatures into integers. Summaries without game links are readable cards, and long related-game lists support search and pagination.
...
- Personal collection goals count seen winners plus unseen players verified on active MLB rosters. Keep original historical totals separate. Collections with no unseen active targets must not be offered as goals, overview recommendations or planner candidates. Apply the same scope to Awards/All-Stars drill-ins. Never infer retirement from award year or mark an injured/unsigned/minor-league player retired merely because they are absent from an active roster. `website.collection_goals` caches all 30 MLB rosters atomically in `cache/collection_rosters.json`, refreshed daily on normal website runs; cache-only builds remain offline. Unknown or >7-day-old roster snapshots cannot supply goals, including when an old static build is opened later. Explicit refresh: `python3 -m baseball_processor.website.collection_goals`, then `npm run build:website`.
